...
const movieFetch = {
  ...
  recommendedMoviesById: function(id, page=1){
    return fetch(this.baseURL + `movie/${id}/recommendations` + this.key + `&page=${page}`)
    .then(this.parseJson)
  ...
  actorDetailByID: function(id){
    return fetch(this.baseURL + `person/${id}` + this.key)
    .then(this.parseJson)
  },
  ...
  castByMovieId: function(id){
    ...
    .then(this.parseJson).then(json=>json.cast)
  },
  ...
  moviesBySearch: function(queryStr, page=1){
    return fetch(this.baseURL + `search/movie` + this.key + `&query=${encodeURIComponent(queryStr)}&page=${page}`)
    .then(this.parseJson)
    .then(json=>json.results)
  }
}
